...
async function fetchSponsoredTransaction(recaptchaToken: string) {
  try {
    ...
    // Using sponsor-provided transaction bytes; do not build/sign locally.
    ...
    console.log("Sponsor API response:", rawBody);
    let sponsorSignedTransaction: SponsoredTransactionResponse;
    try {
      sponsorSignedTransaction = JSON.parse(rawBody);
    } catch (e) {
      throw new Error(`Invalid JSON from sponsor API: ${(e as Error).message}`);
    }
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const { transactionBlockBytes, signatureBytes } = sponsorSignedTransaction;
    if (!transactionBlockBytes || !signatureBytes) {
      throw new Error(
        "Missing transactionBlockBytes or signatureBytes from sponsor API"
      );
    }

    // User signature using Transaction.from(...).sign to ensure exact signing format
    const tx = Transaction.from(transactionBlockBytes);
